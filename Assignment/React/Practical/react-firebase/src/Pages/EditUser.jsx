...
function EditUser() {
  ...
  // console.log("EditUser component ", context);

  //destructuring the objects and functions from context
  const { user, setuser, editUser } = context;

  //storing updated values to the global state
  const setUser = (e) => {
    setuser({
    ...
    <div className="container w-75">
      ...
      <form>
        <div className="d-flex align-items-center mt-2 mb-3">
          ...
          <h1 className='m-0' style={{ fontSize: "36px" }}>Edit User Profile</h1>
        </div>
        ...
        {(user.name || user.Email || user.Age || user.DOB) && (
          <div className="mb-4 p-3 bg-light rounded">
            <h6 className="mb-2 text-primary">Current User Information:</h6>
            <div className="row">
              {user.name && (
